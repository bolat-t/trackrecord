...
- [~] **Phase 3 — Gold + dashboard.** Gold aggregates built (`trackrecord-gold`): `network_kpi`, `line_punctuality`, `hourly_punctuality`, `daily_punctuality` (+ rain), `stop_punctuality`, `weather_delay`. Snapshot: 99.6% on-time (≤5 min), worst line T4, worst station Cronulla Pl 1. *Dashboard next (Databricks AI/BI).*
