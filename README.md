...
- [~] **Phase 3 — Gold + dashboard.** Gold aggregates (`trackrecord-gold`): `network_kpi`, `line_punctuality`, `hourly_punctuality`, `daily_punctuality` (+ rain), `stop_punctuality`, `weather_delay`. **AI/BI (Lakeview) dashboard** created + published via SDK (`trackrecord-dashboard`; spec in `docs/dashboard.lvdash.json`) — KPI counters, worst-lines & by-hour charts, worst-stations table, rain split. Snapshot: 99.6% on-time (≤5 min), worst line T4. *Pending: visual confirm of tiles.*
